...
interface FlatChildRow {
  ...
  subcategory: string;
  category: string; // The child's own category label (deepest level)
  file1_metric: number;
  file2_metric: number;
  difference: number;
...
export function SubSubCategories() {
  ...
  // Reset all local state when new analysis data arrives
  useEffect(() => {
  ...
  // ── Flatten all children from the full 3-tier hierarchy ──
  ...
  // ── Chart data — sorted by difference desc for visual readability ──
  ...
  const handleDownloadCsv = () => {
    ...
    const safeMain = selectedMainCostType.replace(/[^a-z0-9]/gi, "_");
    const safeSub = selectedSubcategory.replace(/[^a-z0-9]/gi, "_");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `sub_subcategories_${projectKey ?? "export"}_${safeMain}_${safeSub}.csv`;
    a.click();
    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-xl font-bold text-white">
          Sub-Subcategory-Level Breakdown
        </h2>
      ...
      {/* ── Section: Sortable 6-Column Table ── */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-900 text-slate-400">
              <tr>
                {/* # column — original index, not sortable */}
