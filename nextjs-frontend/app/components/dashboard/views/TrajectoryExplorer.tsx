    ...
    <Card className="p-4">
        <div className="space-y-1">
            <div className="flex px-4 py-2 font-bold text-slate-400">
                ...
                <div className="w-32 text-right">Value ({metric})</div>
...
function TreeNode({ node, level }: { node: any, level: number }) {
    ...
    const hasChildren = (node.subcategories && node.subcategories.length > 0) || (node.children && node.children.length > 0);
        ...
        <div>
            ...
            >
                <div className="flex flex-1 items-center" style={{ paddingLeft }}>
                    ...
                    <span className="text-sm font-medium text-slate-200">{node.category}</span>
                </div>
                
                <div className="w-32 text-right font-mono text-xs text-slate-400">
                    {/* Using file2 as current value proxy, adjust if needed */}
                    {node.file2_metric?.toLocaleString()}
                </div>
                <div className={`w-32 text-right font-mono text-sm font-bold ${node.difference > 0 ? "text-red-500" : "text-green-500"}`}>
                    {node.difference?.toLocaleString()}
                </div>
            </div>
